  ...
  */
  ...
  switch (type) {
    case 'switch':
      ...
      client.logger.log(`user ${newMember.user.username} switched voice channels: ${oldMember.voiceChannel.name} => ${newMember.voiceChannel.name}`);

      break;

    case 'join':

      client.logger.log(`user ${newMember.user.username} joined voice channel: ${newMember.voiceChannel.name}`);

      break;

    case 'leave':
      client.logger.log(`user ${newMember.user.username} left voice channel: ${oldMember.voiceChannel.name}`);
      break;
